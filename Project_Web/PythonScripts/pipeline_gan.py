...
def run_pipeline_for_file(input_file: Path, model_choice: str, model_mod, model_obj, device, args, working_directory: Path):
	...
	grain_out_dir = working_directory.joinpath('wwwroot', 'output_pipeline_gan')
...
def main():
	...
	if args.model == 'AB':
		mod_path = working_directory.joinpath('PythonScripts', 'gan_ab.py')
		default_ckpt = working_directory.joinpath('PythonScripts', 'GAN_models', 'best_checkpoint_AB_e4.pth')
		
		model_mod = load_module_from_path('gan_ab', mod_path)
	else:
		mod_path = working_directory.joinpath('PythonScripts', 'gan_lab.py')
		default_ckpt = working_directory.joinpath('PythonScripts', 'GAN_models', 'best_checkpoint_LAB_e5.pth')
		model_mod = load_module_from_path('gan_lab', mod_path)
	...
	if input_path.is_dir():
		...
		for f in files:
			res = run_pipeline_for_file(f, args.model, model_mod, model_obj, device, args, working_directory)
			processed.append(res)
	else:
		res = run_pipeline_for_file(input_path, args.model, model_mod, model_obj, device, args, working_directory)
		processed.append(res)
