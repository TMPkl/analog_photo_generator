...
class ResnetBlock(nn.Module):
    """Standardowy ResNet block z możliwością wyboru typu paddingu."""
...
class ResnetGenerator(nn.Module):
    ...
    def init_weights(self, init_type='normal', init_gain=0.02):
        """Prosta inicjalizacja wag (kompatybilna z wieloma projektami)"""
        def init_func(m):  # can be used with apply
            classname = m.__class__.__name__
            if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
                if init_type == 'normal':
